new Vue ({
	...
	data : {
		...
		// films: [], AFTER CODE IS DONE REPLACE IT
		filmId: 0,
		genres: [],
		filterGenres: [],
	...
	methods: {
		browseFilms(a) {
			if (a === 'browse') this.popularMoviesOnly = !this.popularMoviesOnly;
			this.filmId = 0;
			this.showFavs = false;
	...
	computed: {
		...
		relativeFilms() {
			...
			this.singleFilmObj.genres.forEach(g => {
				arr = arr.concat([...this.sortedMovies.filter(f => f.genres.includes(g))]);
			});
			relFilms = [...new Set(arr)];
			return relFilms.splice(0, 5);
		},
		seekName() {
			if (!this.searchMovie) return [];
			return this.films.filter(n => n.name.includes(this.searchMovie));
		}
	},
	beforeMount() {
